...
export function Recipe() {
  const [isLoggedIn, setIsLoggedIn] = useState(
    secureLocalStorage.getItem("logInToken") !== null
  );
  ...
  useEffect(() => {
    recipeService.getRecipe(id).then(
      (res) => {
        ...
        setIngredients(
          res.ingredients.map((e) => {
            return (
              <li key={e.ingredient.id}>
                {e.ingredient.name + ": " + e.amount + e.unitOfMeasure}
              </li>
            );
          })
        );
    ...
    <>
      <div className={classes.wrapper}>
        ...
        <div className={classes.comments_and_rate}>
          ...
          {comment ? (
            <CommentBox recipe_id={id} setRefresh={setRefresh} />
          ) : null}
          <div className={classes.comments}> Comments: </div>
          <ul className={classes.no_bullets}>
            {comments.length > 0 ? [...comments].reverse() : "No comments!"}
          </ul>
        </div>
      </div>
      <Footer sticky={1} />
