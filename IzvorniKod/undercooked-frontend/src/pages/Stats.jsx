...
export function Stats() {
    ...
    useEffect(() => {
        adminService.getBestRatedRecipe().then((data) => { setBestRated(data) }, () => { });
        adminService.getMostActiveUser().then((data) => { setUser(data) }, () => { });
        adminService.getMostSavedRecipe().then(data => { setPopularRecipe(data) }, () => { })
    })
    return <div>
        <header>
            <div>
        ...
        <div>
            ...
            {user !== "" ? <NavLink to={"/profile/" + user.username}> {user.username}</NavLink> : null}
        </div>
        <div>
